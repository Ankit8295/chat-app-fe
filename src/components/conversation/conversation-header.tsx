...
export default function ConversationHeader({
  ...
  return (
    <header className="flex w-full shrink-0 items-center justify-between gap-3 border-b border-border bg-surface px-3 py-2.5 z-20">
      <button
        type="button"
        onClick={onOpenInfo}
        disabled={isLoading}
        aria-label={t("aria-open-conversation-info")}
        className="flex min-w-0 w-fit items-center gap-3 rounded-lg px-1 py-0.5 text-left outline-none transition-colors  disabled:opacity-60 cursor-pointer disabled:cursor-default"
      >
        <Avatar name={name} src={image ?? undefined} size="md" shape="circle" />
        <Typography
          variant="span"
